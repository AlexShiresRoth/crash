...
const CartItem = ({ index, removeFromCart, item, itemToRemove, updateLineItem, shop: { cart } }: Props) => {
	...
	return (
		<div key={index} className={style.item}>
			<Image src={item.variant.image.src} alt={item.title} height={'100%'} width={'100%'} />
			<div className={style.content}>
				<p>{item.title}</p>
				<p>Quantity: {processing ? <LoadingSpinner updateStyle={spinnerStyles} /> : item.quantity}</p>
