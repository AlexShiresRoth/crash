...
const Pages = ({ images }: Props) => {
	...
	const mp3 = new Audio('https://www.soundjay.com/misc/sounds/page-flip-03.mp3');
	...
	const handlePageTurn = (current: number, ceil: number, direction: boolean) => {
		//page turn audio
		mp3.play();
		...
		<>
			...
			>
				<div className={style.back_btn}>
					<button onClick={() => handlePageTurn(currentPages, max, false)}>Back</button>
				</div>
				<div className={style.pages}>{imgsMap[currentPages]}</div>
				<div className={style.next_btn}>
					<button onClick={() => handlePageTurn(currentPages, max, true)}>Next</button>
				</div>
			</div>
		</>
	);
